...
import com.example.tentwenty.App
import com.example.tentwenty.Const.BASE_IMAGE_LARGE
import com.example.tentwenty.Const.YOUTUBE_TRAILER_BASE_URL
import com.example.tentwenty.R
import com.example.tentwenty.ViewModelFactory
import com.example.tentwenty.databinding.FragmentDetailBinding
...
class DetailFragment : Fragment(R.layout.fragment_detail) {
    ...
    private fun initialiseData(movie: Movie) {
        ...
        binding.rating.rating = movie.vote_average!!.toFloat() / 2

        val movieGenreIdList = ArrayList<String>()
        for (i in movie.genre_ids!!.indices) {
            val movieGenreList = movie.genre_ids!!.get(i);
            movieGenreIdList.add(movieGenreList)
        }

        binding.genreIds.text = movieGenreIdList.toString()
